...
ดึงได้จริงแล้ว **11 ร้าน** อีก 2 ร้านติดฝั่งแพลตฟอร์ม (ดูหัวข้อถัดไป)
...
⚠️ **Phase 5 — ห้ามแตะข้อมูลการเงินจนกว่าจะได้รับอนุญาต** (สั่งไว้ 2026-08-04)
ไฟล์ Export ของ Shopee **มี** `ค่าคอมมิชชั่น` / `Transaction Fee` / `ค่าบริการ` อยู่จริง
แต่ตั้งใจไม่ map — มีเทสต์ `test_shopee_does_not_map_financial_columns` กันไว้
ถ้าจะเปิดใช้ ดูวิธีในคอมเมนต์ของ `config/column_maps/shopee.yaml`

ถ้าวันหนึ่งจะทำต่อ วิธีที่ตกลงกันไว้คือ **ให้เจ้าของงาน Export รายงานรายได้เองแล้วส่งไฟล์มา**
ไม่ให้ระบบเข้าเมนูการเงิน (กันกดผิดปุ่มโอนเงิน/เปลี่ยนบัญชีธนาคาร)
...
ทั้ง 2 ร้านตั้ง `enabled: false` + `skip_reason` ไว้ → ขึ้น ⚪ SKIPPED ไม่ใช่ 🔴 FAILED
เหตุผลตามกฎ: แก้ที่โค้ดไม่ได้ ถ้าปล่อยแดงทุกวันสีแดงจะเฝือจนมองข้ามร้านที่พังจริง

| ร้าน | อาการ | ยืนยันเมื่อ |
|---|---|---|
| `shopee_03` Toolspartner | **ไม่มีปุ่ม "ดาวน์โหลด" ในหน้าคำสั่งซื้อ** หน้าโหลดปกติ เห็น 10,794 ออเดอร์ | ผู้ใช้เปิดดูเอง 2026-08-04 (ล็อกอินใหม่แล้วก็ยังไม่มี) |
| `shopee_08` TNLTOOLSTORE | เหมือนกัน (บัญชี `tnltools:setadosuka` เดียวกัน) | ↑ |

**โค้ดพร้อมใช้แล้วทั้งหมด** — วันไหนสิทธิ์เปิด แค่เปลี่ยน `enabled: true` บรรทัดเดียว ไม่ต้องแตะโค้ด
ผู้ใช้จะแจ้งเองเมื่อโหลดได้
