...
DEFS: dict[str, tuple[str, str, str, str, str]] = {
    ...
    "item_subtotal_before_discount": ("DECIMAL(12,2)", "ได้", "เงิน-สินค้า",
                                      "ยอดสินค้าก่อนหักส่วนลด",
                                      "TikTok เท่านั้น — Shopee/Lazada ไม่มีคอลัมน์นี้"),
    "item_discount": ("DECIMAL(12,2)", "ได้", "เงิน-สินค้า", "ส่วนลดระดับรายการ",
                      "Lazada = bundleDiscount (เปิดใช้ 2026-08-11) / Shopee-TikTok ไม่มี"),
    ...
    "payment_discount": ("DECIMAL(12,2)", "ได้", "เงิน-สินค้า", "ส่วนลดจากช่องทางชำระเงิน",
                         "TikTok เท่านั้น — คนละก้อนกับ platform_discount ห้ามบวกรวมกัน"),
    "tax_amount": ("DECIMAL(12,2)", "ได้", "เงิน-สินค้า", "ภาษีที่แพลตฟอร์มแจ้ง",
                   "TikTok เท่านั้น (เปิดใช้ 2026-08-11)"),

    "shipping_fee": ("DECIMAL(12,2)", "ได้", "ขนส่ง", "ค่าส่ง (ระดับออเดอร์)",
                     "เป็นค่าของทั้งออเดอร์ ซ้ำทุกแถวในออเดอร์เดียวกัน"),
    "shipping_fee_seller_discount": ("DECIMAL(12,2)", "ได้", "ขนส่ง",
                                     "ร้านออกค่าส่งให้เท่าไหร่", "TikTok เท่านั้น"),
    "shipping_fee_platform_discount": ("DECIMAL(12,2)", "ได้", "ขนส่ง",
                                       "แพลตฟอร์มออกค่าส่งให้เท่าไหร่", "TikTok เท่านั้น"),
    ...
    "commission_fee": ("DECIMAL(12,2)", "ได้", "ค่าธรรมเนียม", "ค่าคอมมิชชั่น",
                       "Shopee เท่านั้น (เปิดใช้ 2026-08-11) — Lazada/TikTok ไม่มีในรายงานออเดอร์"),
    "transaction_fee": ("DECIMAL(12,2)", "ได้", "ค่าธรรมเนียม", "ค่าธรรมเนียมธุรกรรม",
                        "Shopee เท่านั้น เหตุผลเดียวกับ commission_fee"),
    ...
    "fetched_at": ("DATETIME", "ไม่", "meta", "เวลาที่ระบบดึงข้อมูล",
                   "ไม่ใช่เวลาของออเดอร์ / Dashboard ใช้คอลัมน์นี้วัดว่าร้านไหนข้อมูลค้าง"),

    # ══ เปิดใช้ 2026-08-13 — ทีมชี้ว่าไฟล์ Export ดิบมีคอลัมน์ที่ไม่เคยถูกดึง ══
    # ตรวจกับไฟล์จริง 77 คอลัมน์แล้วจริงตามนั้น ชุดนี้คือส่วนที่เพิ่มเข้ามา
    # ⚠️ เกือบทั้งหมดเป็นของ Shopee — Lazada/TikTok ไม่มีคอลัมน์พวกนี้ในไฟล์ต้นทาง

    "promised_ship_at": ("DATETIME", "ได้", "เวลา", "วันที่คาดว่าจะจัดส่ง",
                         "Shopee เท่านั้น ใช้วัดว่าร้านส่งของทันกำหนดของแพลตฟอร์มไหม"),
    "shipped_at": ("DATETIME", "ได้", "เวลา", "เวลาที่ส่งของออกจากร้าน",
                   "เดิมถูกยุบรวมเข้า order_updated_at แล้วทิ้ง แยกเก็บตั้งแต่ 2026-08-13"),
    "delivered_at": ("DATETIME", "ได้", "เวลา", "เวลาที่ของถึงมือผู้ซื้อ",
                     "เดิมถูกยุบรวมเข้า order_updated_at แล้วทิ้ง เหมือน shipped_at"),
    "completed_at": ("DATETIME", "ได้", "เวลา", "เวลาที่ออเดอร์ปิดสมบูรณ์",
                     "Shopee เท่านั้น หลังหมดสิทธิ์คืนสินค้าแล้ว"),
    "cancelled_at": ("DATETIME", "ได้", "เวลา", "เวลาที่ออเดอร์ถูกยกเลิก", "Shopee เท่านั้น"),
    "settlement_date": ("DATETIME", "ได้", "เวลา", "วันที่เงินเข้า Seller Balance",
                        "เป็น 'วันที่' ไม่ใช่ 'จำนวนเงิน' — ยอดเงินจริงอยู่ในเมนูการเงิน"),

    "parent_sku": ("VARCHAR(128)", "ได้", "สินค้า", "รหัสสินค้าแม่ของผู้ขาย",
                   "สินค้าหลายตัวเลือกใช้ parent_sku เดียวกัน ใช้จับกลุ่มรุ่นเดียวกันได้"),
    "returned_qty": ("DECIMAL(12,2)", "ได้", "สินค้า", "จำนวนที่ส่งคืน",
                     "หักออกจาก quantity เองไม่ได้ ต้องดูสถานะประกอบ"),

    "deal_price": ("DECIMAL(12,2)", "ได้", "เงิน-สินค้า", "ราคาขายต่อชิ้น (หลังลดหน้าร้าน)", ""),
    "net_price": ("DECIMAL(12,2)", "ได้", "เงิน-สินค้า", "ยอดทั้งบรรทัดก่อนหักส่วนลดโค้ด",
                  "ค่ารายบรรทัดจริงตัวเดียวที่ Shopee ให้มา ระบบใช้เป็นน้ำหนักเฉลี่ย revenue_thb"),

    "seller_voucher": ("DECIMAL(12,2)", "ได้", "ส่วนลด-ผู้ขายออก", "โค้ดส่วนลดที่ผู้ขายออกเอง", ""),
    "seller_coin_cashback": ("DECIMAL(12,2)", "ได้", "ส่วนลด-ผู้ขายออก",
                             "Coins Cashback ที่ผู้ขายออก", ""),
    "seller_bundle_discount": ("DECIMAL(12,2)", "ได้", "ส่วนลด-ผู้ขายออก",
                               "ส่วนลด bundle deal ที่ผู้ขายออก", ""),
    "seller_tradein_bonus": ("DECIMAL(12,2)", "ได้", "ส่วนลด-ผู้ขายออก",
                             "โบนัสเครื่องเก่าแลกใหม่ที่ผู้ขายออก", ""),

    "platform_voucher": ("DECIMAL(12,2)", "ได้", "ส่วนลด-แพลตฟอร์มออก",
                         "โค้ดส่วนลดที่ Shopee ออกให้",
                         "ก้อนใหญ่ที่สุดในบรรดาส่วนลด เดิมไม่ถูกดึงเลย ทำให้ "
                         "platform_discount ต่ำกว่าจริงราว 44 เท่า"),
    "platform_bundle_discount": ("DECIMAL(12,2)", "ได้", "ส่วนลด-แพลตฟอร์มออก",
                                 "ส่วนลด bundle deal ที่ Shopee ออก", ""),
    "coin_discount": ("DECIMAL(12,2)", "ได้", "ส่วนลด-แพลตฟอร์มออก", "ส่วนลดจากการใช้เหรียญ", ""),
    "tradein_discount": ("DECIMAL(12,2)", "ได้", "ส่วนลด-แพลตฟอร์มออก",
                         "ส่วนลดเครื่องเก่าแลกใหม่", ""),
    "tradein_bonus": ("DECIMAL(12,2)", "ได้", "ส่วนลด-แพลตฟอร์มออก",
                      "โบนัสเครื่องเก่าแลกใหม่", ""),
    "voucher_total": ("DECIMAL(12,2)", "ได้", "ส่วนลด-แพลตฟอร์มออก", "โค้ดส่วนลด (รวม)",
                      "ในไฟล์ Export ที่เราได้ = 0 ทุกแถว ไม่ใช่ผลรวมของช่องอื่น"),

    "estimated_shipping_fee": ("DECIMAL(12,2)", "ได้", "ขนส่ง", "ค่าจัดส่งโดยประมาณ",
                               "ต่างจาก shipping_fee ซึ่งเป็นส่วนที่ผู้ซื้อจ่ายจริง "
                               "ส่วนต่างคือที่ Shopee ออกให้"),
    "return_shipping_fee": ("DECIMAL(12,2)", "ได้", "ขนส่ง", "ค่าจัดส่งสินค้าคืน", ""),
    "shipping_method": ("VARCHAR(64)", "ได้", "ขนส่ง", "วิธีการจัดส่ง", ""),

    "installation_fee_buyer": ("DECIMAL(12,2)", "ได้", "ค่าธรรมเนียม",
                               "ค่าติดตั้งที่ผู้ซื้อจ่าย", ""),
    "installation_fee_actual": ("DECIMAL(12,2)", "ได้", "ค่าธรรมเนียม",
                                "ค่าติดตั้งจริงที่ผู้ให้บริการคิด", ""),

    "item_paid_by_buyer": ("DECIMAL(12,2)", "ได้", "เงิน-ออเดอร์",
                           "ค่าสินค้าที่ผู้ซื้อจ่าย (ไม่รวมค่าส่ง)",
                           "เป็นค่าระดับออเดอร์ ซ้ำทุกแถว ห้าม SUM ตรง ๆ / "
                           "ยืนยันแล้ว: ค่านี้ + ค่าจัดส่ง = total_amount ตรง 100%"),

    "district": ("VARCHAR(64)", "ได้", "ผู้ซื้อ", "เขต/อำเภอผู้รับ",
                 "ระดับพื้นที่ ไม่ใช่ที่อยู่จริง — ชื่อ/เบอร์/ที่อยู่ ไม่ถูกดึงเข้าระบบ"),
    "country": ("VARCHAR(64)", "ได้", "ผู้ซื้อ", "ประเทศผู้รับ", ""),
    # ยังอธิบายไว้แม้ถูกถอดออกจาก EXCEL_COLUMNS แล้ว — ไฟล์เก่ายังมีคอลัมน์นี้อยู่
    "postcode": ("VARCHAR(16)", "ได้", "ผู้ซื้อ", "รหัสไปรษณีย์ผู้รับ — ว่างเสมอโดยตั้งใจ",
                 "ระบบลบทิ้งทุกรอบ (PII_FIELDS ใน privacy.py) รหัสไปรษณีย์ + เขต/อำเภอ "
                 "ชี้ตำแหน่งได้แคบเกินไป ถอดออกจากไฟล์ตั้งแต่ 2026-08-13"),

    "order_type": ("VARCHAR(64)", "ได้", "ธง/ประเภท", "ประเภทคำสั่งซื้อ", ""),
    "fulfilled_by_platform": ("VARCHAR(16)", "ได้", "ธง/ประเภท",
                              "ออเดอร์ที่แพลตฟอร์มจัดส่งให้", ""),
    "owned_by_platform": ("VARCHAR(16)", "ได้", "ธง/ประเภท", "สินค้าที่แพลตฟอร์มเป็นเจ้าของ", ""),
    "in_bundle_deal": ("VARCHAR(16)", "ได้", "ธง/ประเภท", "เข้าร่วมแคมเปญ bundle deal ไหม", ""),
    "hot_listing": ("VARCHAR(16)", "ได้", "ธง/ประเภท", "ประกาศที่ถูกจัดเป็น Hot Listing", ""),
    "tax_invoice_requested": ("VARCHAR(16)", "ได้", "ธง/ประเภท", "ผู้ซื้อขอใบกำกับภาษีไหม",
                              "เก็บแค่ 'ขอ/ไม่ขอ' — ชื่อ ที่อยู่ เลขผู้เสียภาษี ไม่ถูกดึงเข้าระบบ"),
    "tax_invoice_type": ("VARCHAR(64)", "ได้", "ธง/ประเภท", "ประเภทใบกำกับภาษี", ""),
    "seller_note": ("VARCHAR(512)", "ได้", "meta", "บันทึกของผู้ขาย",
                    "เป็นบันทึกฝั่งร้าน ไม่ใช่หมายเหตุของผู้ซื้อ "
                    "(หมายเหตุผู้ซื้อไม่ถูกดึงเพราะมักมีที่อยู่/เบอร์โทรปนมา)"),
}
